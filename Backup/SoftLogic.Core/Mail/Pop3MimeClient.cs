	...
	namespace Mail
	{
		namespace Pop3
		{
			...
			public class Pop3MimeClient : Pop3MailClient
			{
				...
				/// </summary>
				private bool parentBoundaryFound(string response, string parentBoundaryStart, string parentBoundaryEnd, ref MimeEntityReturnCode boundaryMimeReturnCode)
				{
				...
				public DateTime ConvertToDateTime(string dateTime)
				{
					...
					try
					{
						...
						//convert to DateTime
                        if (!DateTime.TryParse(cleanDateTime, culture, DateTimeStyles.AdjustToUniversal | DateTimeStyles.AllowWhiteSpaces, out ReturnDateTime))
						{
