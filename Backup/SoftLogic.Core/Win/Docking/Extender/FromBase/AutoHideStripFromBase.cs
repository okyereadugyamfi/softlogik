...
#region Customizable Properties
		...
		private void DrawTabStrip(Graphics g, DockState dockState)
		{
			...
			if (dockState == DockState.DockLeftAutoHide || dockState == DockState.DockRightAutoHide)
			{
				...
				matrixRotated.RotateAt(90F, new PointF(System.Convert.ToSingle(rectTabStrip.X) + System.Convert.ToSingle(rectTabStrip.Height) / 2, System.Convert.ToSingle(rectTabStrip.Y) + System.Convert.ToSingle(rectTabStrip.Height) / 2));
		...
		private void CalculateTabs(DockState dockState)
		{
			...
			using (Graphics g = CreateGraphics())
			{
				...
				foreach (AutoHidePane pane in GetPanes(dockState))
				{
					...
					foreach (AutoHideTabFromBase tab in pane.Tabs)
					{
						int width = imageWidth + ImageGapLeft + ImageGapRight + System.Convert.ToInt32(g.MeasureString(tab.Content.DockHandler.TabText, Font).Width) + 1 + TextGapLeft + TextGapRight;
						if (width > maxWidth)
						{
							maxWidth = width;
						}
		...
		private Rectangle GetTransformedRectangle(DockState dockState, Rectangle rect)
		{
			...
			pts[0].X = System.Convert.ToSingle(rect.X) + System.Convert.ToSingle(rect.Width) / 2;
			pts[0].Y = System.Convert.ToSingle(rect.Y) + System.Convert.ToSingle(rect.Height) / 2;
			Rectangle rectTabStrip = GetLogicalTabStripRectangle(dockState);
			Matrix matrix = new Matrix();
			matrix.RotateAt(90F, new PointF(System.Convert.ToSingle(rectTabStrip.X) + System.Convert.ToSingle(rectTabStrip.Height) / 2, System.Convert.ToSingle(rectTabStrip.Y) + System.Convert.ToSingle(rectTabStrip.Height) / 2));
			matrix.TransformPoints(pts);
			return new Rectangle(System.Convert.ToInt32((pts[0].X - System.Convert.ToSingle(rect.Height) / 2 + 0.5F)), System.Convert.ToInt32((pts[0].Y - System.Convert.ToSingle(rect.Width) / 2 + 0.5F)), rect.Height, rect.Width);
		}
