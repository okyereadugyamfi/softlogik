...
namespace SoftLogik.Win.UI
{
	...
	/// </summary>
	public class VisualTabOrderManager
	{
		...
		public VisualTabOrderManager(Control container)
		{
		...
		private VisualTabOrderManager(Control container, int curTabIndex, Hashtable schemeOverrides)
		{
			this.container = container;
			...
			try
			{
				...
				foreach (Control tempLoopVar_c in controlArraySorted)
				{
					...
					if (c.Controls.Count > 0)
					{
						...
						curTabIndex = (new VisualTabOrderManager(c, curTabIndex, schemeOverrides)).SetTabOrder(childScheme);
					}
				}
